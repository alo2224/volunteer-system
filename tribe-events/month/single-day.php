...
<?php while ( $day['events']->have_posts() ) : $day['events']->the_post(); ?>
	<?php 
        $current_post_id = $day['events']->post->ID;
        $attendees = get_post_meta($current_post_id, 'uczestnicy', true);
        if(!empty($attendees)){
            foreach($attendees as $key => $value){
                $wolontariusz_id = get_user_meta(get_current_user_id(), 'wolontariusz_id', true);
                if($key == $wolontariusz_id){
                    echo "Uzytkownik zapisany";
                    //TODO Wyswietlanie wydarzen dla danego uzytkownika
                }
            }
        }
